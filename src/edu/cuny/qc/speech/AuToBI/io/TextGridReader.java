...
public class TextGridReader extends AuToBIWordReader {
  ...
  public List<Word> readWords() throws IOException, AuToBIException {
    ...
    do {
      ...
      } else if (tier.name != null && (tier.name.equals("Word") || tier.name.equals("words") || tier.name.equals("orthographic"))) {
        words_tier = tier;
      }
      ...
      } else if (tier.name != null && (tier.name.equals("tones") || tier.name.endsWith("accents") )) {
          if(tones_tier==null)
            tones_tier = tier;
          else{
              
              ((TextGridTier) tones_tier).mergeWith(tier);
          }
      }
      ...
      } else if (tier.name != null && (tier.name.equals("breaks") || tier.name.equals("Bi") || tier.name.equals("words"))) {
          
        if(tier.name.equals("words")){
            breaks_tier  = new TextGridTier();
            ((TextGridTier)breaks_tier).wordsToBreaksForDirndl(words_tier);
            
        }else breaks_tier = tier;
      }
    ...
    if (tones_tier != null) {
      ...
      if (breaks_tier == null || breaks_tier.getRegions().size() == 0) {
        ...
        //AlignmentUtils.copyBreaksByTime(words, breaks_tier.getRegions());
        try {
          
            if(breaks_tier.name.equals("Bi")){
                AlignmentUtils.copyBreaksByTime(words, breaks_tier.getRegions());
            }else{
