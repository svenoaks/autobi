...
import edu.cuny.qc.speech.AuToBI.util.UttsFileComparator;
...
public class AuToBI {
    ...
    public HashMap<String, HashMap<String, ArrayList<Word>>> lWords;
    ...
    public void generatePredictions(String task, FeatureSet fs) throws AuToBIException {
        if (tasks.containsKey(task)) {
            AuToBITask autobi_task = tasks.get(task);
            ClassifierUtils.generatePredictions(autobi_task.getClassifier(),
    ...
    public void run() {
        try {
            ...
            if (hasParameter("out_file")) {
                String result = evaluateDetectionPerformance("pitch_accent_detection", autobi_fs);
                AuToBIFileWriter writer = new AuToBIFileWriter(getOptionalParameter("out_file") + "." + "pitch");
                writer.write(result);
                writer.close();


                result = evaluateDetectionPerformance("intermediate_phrase_boundary_detection", autobi_fs);
                writer = new AuToBIFileWriter(getOptionalParameter("out_file") + "." + "intermediate");
                writer.write(result);
                writer.close();

                result = evaluateDetectionPerformance("intonational_phrase_boundary_detection", autobi_fs);
                writer = new AuToBIFileWriter(getOptionalParameter("out_file") + "." + "intonational");
                writer.write(result);
                writer.close();
            }
    ...
    public List<Word> myBabelRun(String WAVDIR, String modelIPBFilename, String modelPitchFilename, String wav_filename, HashMap<String, ArrayList<Word>> utts) {


        List<Word> taggedWords = new ArrayList<Word>();

        try {

            String intonational_phrase_detector_filename = modelIPBFilename;
            String pitch_accent_detector_filename = modelPitchFilename;

            AuToBIClassifier intonational_phrase_boundary_detector = ClassifierUtils.readAuToBIClassifier(intonational_phrase_detector_filename);
            AuToBIClassifier pitch_accent_detector = ClassifierUtils.readAuToBIClassifier(pitch_accent_detector_filename);

            WavReader reader = new WavReader();
            WavData wav = reader.read(WAVDIR + wav_filename + ".wav");

            UttsFileComparator comp = new UttsFileComparator();
            TreeMap<String, ArrayList<Word>> sortedUtts = new TreeMap<String, ArrayList<Word>>(comp);
            sortedUtts.putAll(utts);

            List<Word> words = new ArrayList<Word>();

            for (Entry<String, ArrayList<Word>> e : sortedUtts.entrySet()) {
            ...
            AuToBITask t = new AuToBITask();
            t.setClassifier(intonational_phrase_boundary_detector);
            t.setTrueFeature("nominal_IntonationalPhraseBoundary");
            t.setHypFeature("hyp_IP_location");
            t.setConfFeature("hyp_IP_location_conf");
            t.setDistFeature("hyp_IP_location_dist");
            t.setFeatureSet(new IntonationalPhraseBoundaryDetectionFeatureSet());
            tasks.put("intonational_phrase_boundary_detection", t);

            t = new AuToBITask();
            t.setClassifier(pitch_accent_detector);
            t.setTrueFeature("nominal_PitchAccent");
            t.setHypFeature("hyp_pitch_accent");
            t.setConfFeature("hyp_pitch_accent_conf");
            t.setDistFeature("hyp_pitch_accent_dist");
            t.setFeatureSet(new PitchAccentDetectionFeatureSet());
            tasks.put("pitch_accent_detection", t);
            ...
            for (AuToBITask task : tasks.values()) {
                ClassifierUtils.evaluateClassification(task.getHypFeature(), task.getTrueFeature(), autobi_fs);
            }

            taggedWords.addAll(autobi_fs.getDataPoints());
    ...
    public List<Word> myBabelRunAll(String WAVDIR, String modelIPBFilename, String modelPitchFilename, HashMap<String, HashMap<String, ArrayList<Word>>> lWords) {
        ...
        try {
            ...
            for (Entry<String, HashMap<String, ArrayList<Word>>> entry : lWords.entrySet()) {


                String wav_filename = entry.getKey();
                WavReader reader = new WavReader();
                WavData wav = reader.read(WAVDIR + wav_filename + ".wav");

                HashMap<String, ArrayList<Word>> utts = entry.getValue();
                UttsFileComparator comp = new UttsFileComparator();
                TreeMap<String, ArrayList<Word>> sortedUtts = new TreeMap<String, ArrayList<Word>>(comp);
                sortedUtts.putAll(utts);
