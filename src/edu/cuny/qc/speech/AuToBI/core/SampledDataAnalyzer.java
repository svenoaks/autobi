...
public abstract class SampledDataAnalyzer {
  ...
  protected Pair<Integer, Double> shortTermAnalysis(double time_step, double dt_window) {
    ...
    double ourMidTime = wav.t0 +  0.5 * wav.getDuration() - 0.5 * wav.getFrameSize();
    double thyDuration = pair.first*time_step;
    pair.second = ourMidTime - 0.5 * thyDuration + 0.5 * time_step;
    return pair;
    
  }
  
  protected Pair<Integer, Double> shortTermAnalysisNew(double time_step, double dt_window) {
    
    double myDuration = wav.getDuration();
    if (dt_window > myDuration)
        return null;
    double numberOfFrames = Math.floor ((myDuration - dt_window) / time_step) + 1;
    if(numberOfFrames >= 1)
        return null;
    double ourMidTime = wav.t0 - 0.5 * wav.getFrameSize() + 0.5 * myDuration;
    double thyDuration = numberOfFrames * time_step;
    double firstTime = ourMidTime - 0.5 * thyDuration + 0.5 * time_step;
    
    return new Pair(numberOfFrames,firstTime);
